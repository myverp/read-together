...
export type DrawingInput = Omit<Drawing, 'seat' | 'color'>;
...
export function isDrawingInput(value: unknown): value is DrawingInput {
  ...
  if (!d.strokes.every(stroke => stroke && typeof stroke === 'object' && finite(stroke.width, 1, 16) && Array.isArray(stroke.points) && stroke.points.length > 0 && stroke.points.length <= 1000 && stroke.points.every(p => point(p, d.page.width, d.page.height)))) return false;
